define([
	...
	return {
		...
		init: function() {
			...
			var menus = []
			var menuObj = {}
			getMenuList(http, config, function(result) {
				menus = result
...
//							'<a href="javascript:void(0)" class="item-delete">删除</a>' +
			...
			$('#addSubmit').on('click', function() {
				...
				if (nodeObj.parent) {
					...
					table.treetable('loadBranch', table.treetable('node', parentNode.id), $$.treetableRowGenerator(nodeObj, getTreeTableColumns(util, $$, menus, menuObj), parentNode.id, 'add'))
				} else {
					menus.push(nodeObj)
					table.treetable('loadBranch', null, $$.treetableRowGenerator(nodeObj, getTreeTableColumns(util, $$, menus, menuObj), '', 'add'))
				}
			...
			$('#menuUpload').on('click', function() {
				http.post(config.api.menu.save + '?system=' + config.system, {
					data: JSON.stringify(menus)
				}, function(result) {
					getMenuList(http, config)
...
function getMenuList(http, config, callback) {
	http.post(config.api.menu.load, {
		...
		flatten([], result).map(function(item) {
